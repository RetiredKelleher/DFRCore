...
//! @brief DFR::RealTimeSimClock inherits from DFR::SimClock and implements a real-time simulation clock that advances based on wall-clock time.
//! @details The RealTimeSimClock class provides functionality for advancing simulation time in real-time mode
...
{
public:
    //! @param[in] initialTime        Starting simulation time.
    //! @param[in] syncRate           Ratio of sim time to wall time (1.0 = real-time, 2.0 = 2x faster).
    //! @param[in] wallClockLimitRate Max sim advance per tick as a multiple of tickInterval. Must be >= 1.0.
    //! @param[in] tickInterval       Wall-clock sleep duration per tick in seconds.
