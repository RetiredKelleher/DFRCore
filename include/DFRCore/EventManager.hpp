...
//! @brief DFR::EventManager class definition, responsible for managing events in
//! the DFRSimArch framework.
//! @author Thomas Kelleher
//! @date 2024-06-01
//!
//! The DFR::EventManager class provides functionality for scheduling, peeking,
//! and retrieving events based on their simulation time and priority. It maintains
//! a priority queue of events and ensures thread-safe access to the event queue.
//! The EventManager is designed to be used by the SimEngine to manage the execution
//! of events in the simulation loop.
...
//! @brief The DFR::EventManager class is responsible for managing events in the DFRCore framework. It provides methods for adding, peeking, retrieving, and removing events based on their simulation time and priority. The EventManager maintains a priority queue of events and ensures thread-safe access to the event queue using a recursive mutex. This class is designed to be used by the SimEngine to manage the execution of events in the simulation loop.
class EventManager
{
public:
    EventManager();
