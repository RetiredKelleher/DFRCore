...
// @date    2024-06-01
// @brief   Defines the DFR::Event class and related event types for the Discrete Event Simulation framework. 
//! The DFREvent class represents a scheduled event in the simulation, with
//! properties such as simulation time, priority, and execution status. The
//! DFROneShotEvent and DFRRepeatingEvent classes are derived from DFREvent
//! and provide specific implementations for one-shot and repeating events,
//! respectively. The EventManager class (not defined here) is responsible for
//! managing the scheduling and execution of these events in the simulation.
///////////////////////////////////////////////////////////////////////////////
...
//! @brief The DFR::Event class represents a scheduled event in the DFRCore framework. It contains properties such as simulation time, priority, and execution status, and provides a virtual Execute() method that can be overridden by derived classes to define specific event behaviors.
//! The DFR::Event class serves as a base class for specific event types like DFROneShotEvent and DFRRepeatingEvent, which can implement their own execution logic while leveraging the common properties and scheduling capabilities of the DFREvent class. The EventManager class (not defined here) is responsible for managing the scheduling and execution of these events in the simulation.
class Event
{
...
public:
    ...
    //! @brief Constructs a DFREvent instance with the specified simulation time and priority. The event is initialized with a default event counter of 0 and an execution flag set to true.
    //! @param[in] simtime The simulation time at which the event is scheduled to execute. Default is 0.0.
    //! @param[in] priority The priority of the event, where higher values indicate higher priority. Default is 16.
    ...
    //! @brief Get the priority of the event. Higher priority events are executed before lower priority events when they are scheduled for the same simulation time.
    ...
    //! @param [in] New simulation time to be assigned to the clock.
 ...
 protected:
    ...
    int    mPriority{0};
    bool   mExecuteFlag{true};
};

//////////////////////////////////////////////////////////////////////////////////////
//! @brief Template class for DFREvent that allows for flexible execution functions with varying return types and argument lists.
//! This class serves as a base for specific event types like DFROneShotEvent and DFRRepeatingEvent,
//! which can define their own execution logic while leveraging the common properties
//! and scheduling capabilities of the DFREvent class.
...
//! implemented using this base class. The DFREventT class is designed to be inherited
...
//! features provided by the DFREvent base class. 
//////////////////////////////////////////////////////////////////////////////////////
...
//! The DFROneShotEvent class inherits from DFREventT with a void return type
//! and takes a reference to a DFREvent as an argument for the execution function.
...
//! The DFRRepeatingEvent class inherits from DFREventT with an EventStatus return type and takes a reference to a DFREvent as an argument for the execution function. When the Execute method is called, it executes the provided function and returns the
